...
def execute_all_operations(df, config, FLAGS):
    ...
    for operation in config["operation"]:
        print(
            f'Anonymizing the column \'{operation["column"]}\' for the type \'{operation["type"]}\' ...'
        )
        ...
        if operation["type"] == "encrypt":
            dataProtection.encrypt(
                operation, config["output_file"], config["encryption_key_file"]
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--ipex", action="store_true", help="Use IPEX optimized inference."
    )
    parser.add_argument(
        "-m",
        "--modin",
    ...
    if FLAGS.modin:
        ...
        if FLAGS.verbose:
            FLAGS.verbose = False
            print(
                "Verbose option is not allowed with Modin, progress bars will be disabled"
            )
