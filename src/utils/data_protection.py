...
class DataProtection:
    ...
    def __init__(self, df, FLAGS) -> None:
        ...
        if FLAGS.verbose:
            tqdm(
                position=0,
                leave=True,
                bar_format="{desc}: {elapsed} {percentage:3.0f}%|{bar}|{r_bar}",
            ).pandas(desc="Progress", ascii=self.tqdmbar, bar_format=self.tqdmbarformat)
    ...
    def mask_string(self, operation) -> None:
        ...
        with CodeTimer("mask_string"):
            self.df[operation["column"]] = self.df[operation["column"]].progress_apply(
                self.utilities.obfuscate_string, slen=length
            )
    ...
    def mask_numeric(self, operation) -> None:
        ...
        length = (
            operation["len"]
            if operation["len"]
            else self.utilities.col_mean_length(self.df[operation["column"]].to_numpy())
        )
        with CodeTimer("mask_numeric"):
            self.df[operation["column"]] = self.df[operation["column"]].progress_apply(
                self.utilities.obfuscate_number, nlen=length
            )
    ...
    def mask_phone(self, operation) -> None:
        ...
        with CodeTimer("mask_numeric_with_prefix"):
            self.df[operation["column"]] = (
                operation["prefix"]
                + "-"
                + self.df[operation["column"]].progress_apply(
                    self.utilities.obfuscate_number, nlen=length
                )
            )
    ...
    def mask_ip(self, operation) -> None:
        ...
        with CodeTimer("mask_ipv4_address"):
            self.df[operation["column"]] = self.df[operation["column"]].progress_apply(
                self.utilities.obfuscate_ipv4_address
            )
    ...
    def mask_email(self, operation) -> None:
        ...
        with CodeTimer("mask_email_address"):
            self.df[operation["column"]] = self.df[operation["column"]].progress_apply(
                self.utilities.generate_random_emails
            )
    ...
    def mask_invoice_number(self, operation) -> None:
        ...
        with CodeTimer("mask_formatted_numeric"):
            self.df[operation["column"]] = self.df[operation["column"]].progress_apply(
                self.utilities.obfuscate_number_format,
                format="999-999-999-999-999-99999",
                slen=length,
            )
    ...
    def mask_ssn(self, operation) -> None:
        ...
        with CodeTimer("string_hash"):
            self.df[operation["column"]] = self.df[operation["column"]].progress_apply(
                self.utilities.md5hash, data="abcdefghij123456"
            )

    def mask_with_char(self, operation) -> None:
        self.validate.validate_mask_with_char(operation)
        if "start" not in operation.keys():
            start = 0
        if "end" not in operation.keys():
            end = 0
        if "char" not in operation.keys():
            char = "X"
        with CodeTimer("mask_string_partly"):
            self.df[operation["column"]] = self.df[operation["column"]].progress_apply(
                self.utilities.mask_with_char, start=start, end=end, char=char
            )

    def encrypt(self, operation, data_file_path, key_file_path) -> None:
        self.validate.validate_encrypt(operation)
        key = self.utilities.generate_store_encyption_key(
            operation["column"], key_file_path, data_file_path
        )
        with CodeTimer("encrypt_string"):
            self.df[operation["column"]] = self.df[operation["column"]].progress_apply(
                self.utilities.encrypt, key=key
            )
